...
class USB_X_Session(ControlSurface):
    __doc__ = " Script for USB_X_Session in APC emulation mode "

    _active_instances = []
    def _combine_active_instances():
        track_offset = 0
        scene_offset = 0
        for instance in USB_X_Session._active_instances:
    ...
    def _do_combine(self):
        if self not in USB_X_Session._active_instances:
            USB_X_Session._active_instances.append(self)
            USB_X_Session._combine_active_instances()


    def _do_uncombine(self):
        if ((self in USB_X_Session._active_instances) and USB_X_Session._active_instances.remove(self)):
    ...
    def _setup_session_control(self):
        is_momentary = True
        self._session = SpecialSessionComponent(4, 1)
        ...
        self._scene_launch_buttons = [self._note_map[SCENELAUNCH[index]] for index in range(1) ]
